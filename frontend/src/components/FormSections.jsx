...
const Input = ({ id, label, value, onChange, onTranscription, onStatusChange, ...props }) => (
    <div>
        ...
        <div className="relative mt-1">
            <input type="text" id={id} name={id} value={value} onChange={onChange} className="input-text pr-10" {...props} />
...
const Textarea = ({ id, label, value, onChange, onTranscription, onStatusChange, ...props }) => (
    <div>
        <label htmlFor={id} className="block text-sm font-medium text-gray-700">{label}</label>
        <div className="relative mt-1">
            <textarea id={id} name={id} value={value} onChange={onChange} className="input-text pr-10" {...props}></textarea>
            <MicButton fieldId={id} onTranscription={onTranscription} onStatusChange={onStatusChange} />
        </div>
    </div>
);
    ...
    <div className="flex space-x-4">
        {options.map(opt => (
            <label key={opt.value} className="radio-label">
                <input type="radio" name={name} value={opt.value} checked={value === opt.value} onChange={onChange} className="radio-input mr-2" />
                {opt.label}
            </label>
        ))}
    </div>
...
// --- FORM SECTION COMPONENTS (MODIFIED) ---
...
export const EventDetails = ({ data, onChange, onTranscription, onStatusChange }) => (
    <fieldset className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <Input id="event_date" label="Date:" value={data.event_date} onChange={onChange} onTranscription={onTranscription} onStatusChange={onStatusChange} />
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div>
                 ...
                 <div className="mt-2 flex space-x-4">
                     <RadioGroup name="gender" value={data.gender} onChange={onChange} options={[{value: 'male', label: 'Male'}, {value: 'female', label: 'Female'}, {value: 'other', label: 'Other'}]}/>
                 </div>
            </div>
        </div>
...
export const MedicalHistory = ({ data, onChange, onTranscription, onStatusChange }) => (
     <fieldset className="p-4 space-y-4">
        ...
        <div className="space-y-2">
            <p className="text-sm font-medium text-gray-700">Past Medical History:</p>
...
export const GingivaOralHealth = ({ data, onChange, onTranscription, onStatusChange }) => (
    <fieldset className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
             ...
             <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">Chronic Gingivitis:</label>
             ...
             <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">Chronic Periodontitis:</label>
