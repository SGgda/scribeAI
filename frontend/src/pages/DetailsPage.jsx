...
function DetailsPage() {
  ...
  const processAudio = async () => {
    setStatus({
      message: "Transcribing and analyzing... Please wait.",
      color: "blue",
    });
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorResult.error || `Server error: ${response.statusText}`
        );
      }
  ...
  const handleSubmitRecord = () => {
    ...
    const isFormEmpty = Object.values(currentFormData).every(
      (val) => val === ""
    );
    if (isFormEmpty) {
      setStatus({
        message: "Form is empty, nothing to submit.",
        color: "blue",
      });
      return;
    }
    ...
    try {
      allRecords = JSON.parse(
        localStorage.getItem("dentalCampRecords") || "[]"
      );
    } catch (e) {
    ...
    try {
      ...
      setStatus({
        message: "Error saving to local record cache.",
        color: "red",
      });
    }
  };
  ...
  const handleClearCache = () => {
    if (
      window.confirm(
        "Are you sure you want to clear all submitted data? This will start a new CSV file."
      )
    ) {
      localStorage.removeItem("dentalCampRecords"); // clear saved records
      if (formRef.current) {
        formRef.current.startNewDay(); // clear fixed fields in the form
      }
      setStatus({
        message: "Cleared local data. Ready to start a new session.",
        color: "blue",
      });
    }
  };
